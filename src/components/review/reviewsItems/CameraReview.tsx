...
export default function CameraReview() {
  ...
  const selectedCameras = bundle.cameras.flatMap((camera) => {
  ...
  return (
    <div className="space-y-[8px] pt-[15px] border-t border-[#CED6DE]">
      {/* Title */}
      <h3 className="text-[#A8B2BD] text-[12px] tracking-[3%]">CAMERA</h3>
        ...
        >
          ...
          <div className="flex gap-2 sm:gap-[16px] items-center justify-end flex-shrink-0">
            <Counter
              ...
              variant="review"
              onIncrease={() =>
                updateCameraQuantity(item.product.id, item.variant, "increase")
              }
              onDecrease={() =>
                updateCameraQuantity(item.product.id, item.variant, "decrease")
              }
            />

            <div
              className="flex flex-col xl:flex-row xl:gap-[10px]
              items-end sm:items-center font-gilroy font-medium
